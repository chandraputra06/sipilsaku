    ...
    <section class="bg-[#FCF5EE] py-10">
        <div class="mx-auto max-w-[1240px] px-6 lg:px-8">
            ...
            <div class="rounded-[22px] bg-white p-6 shadow-sm">
                <form id="adminEbookSearchForm" method="GET" action="{{ route('admin.ebooks.index') }}" class="mb-6">
                    <input id="adminEbookSearchInput" type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari judul atau penulis..." class="auth-input max-w-md">
                </form>
                        ...
                        <tbody>
                            @forelse ($ebooks as $ebook)
                                <tr class="border-b border-[#F1E8DE] text-sm text-[#4D371F]">
                                    <td class="px-3 py-3">
                                        <img src="{{ $ebook->cover ? asset('storage/' . $ebook->cover) : asset('assets/image/ebook-placeholder.png') }}"
                                            alt="{{ $ebook->title }}" class="h-14 w-10 rounded object-cover">
                                    </td>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const form = document.getElementById('adminEbookSearchForm');
                const input = document.getElementById('adminEbookSearchInput');
